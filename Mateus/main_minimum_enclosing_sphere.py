...
try:
    with open(r"SAVI_Trabalho_pratico_1_108967_108510\Mateus\output_opti.txt", "r") as file:
        content = file.read()
    ...
    print("Loaded external transform from file and computed pcd_source_opti.")

# caso o fincheiro não exista usa point cloud 2 original
except Exception as e:   
    print("No external transform file found or parse error; using original pcd2 as aligned placeholder.")
    pcd_source_opti = copy.deepcopy(pcd2)
...
def error_target_source(pcd, sphere_params, lambda_r=20):
    points = np.asarray(pcd.points)
    ...
    return float(error_points + error_radius)
...
print("Initial sphere params:", initial_params)
...
# -------------- Função objetivo (agora escreve shared para a UI) --------------
def objectiveFunction(params, shared_mem):
    # params = [xc, yc, zc, r]
    center_with_radius = params
    # gerar pontos da esfera (superfície) a partir de params (determinística para visualização)
    ...
    # calcular o erro (mesma função que usavas)
...
def run_optimization(initial_params, shared_local):
    print("Starting sphere optimization thread...")
    res = least_squares(partial(objectiveFunction, shared_mem=shared_local), initial_params, verbose=2, max_nfev=200)
...
# aplicar camera do teu 'view' para manter consistência
...
if shared.get('final_params') is not None:
    ...
    pcd_sphere_final = create_sphere_pointcloud_from_array(final_params, n_points=2000, color=[0.8,0.2,0.2])
else:
